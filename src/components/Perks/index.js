  ...
  return (
    <ul className="flex flex-wrap justify-center -mr-6 -mt-6">
      {perksData.site.siteMetadata.perks.map(perk => {
        return (
          <li key={perk.component} className="mb-6 pr-6 flex flex-col w-1/2 md:w-auto">
            <i className="text-2xl md:text-5xl">{componentMap[perk.component]}</i>
            <span className="ml-2 text-gray-mediumLight text-lg md:text-xl">{perk.text}</span>
          </li>
        );
      })}
    </ul>
