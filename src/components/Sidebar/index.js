...
const topLevelClasses = `font-semibold border-b-2 border-gray pt-8 mb-4`;
const nonTopLevelClasses = `font-medium hover:text-gray-medium pl-2 rounded-md`;


export default forwardRef(({ headings, menuIsOpen, toggleMenu, breadcrumbText = "", breadcrumbUrl = "" }, sidebarRef) => {
  ...
  useLayoutEffect(() => {
    ...
    const activeItem = activeHeadingRef.current || activeSubheadingRef.current;
  ...
  const SubHeadingList = ({ subHeadings, isPrimaryHeading, dampen = true }) => {
    return (
      <ul className={`${isPrimaryHeading ? '' : 'ml-6'}`}>
        {subHeadings.map(subHeading => {
          ...
          return (
            <li key={subHeading.path} ref={isActive ? activeSubheadingRef : null} >
              <span className={`mx-5 block`}>
                <a
                  onClick={(e) => scrollTo(e, setPath, toggleMenu)}
                  href={subHeading.path}
                  className={`block text-lg text-gray-mediumLight py-1 font-light ${nonTopLevelClasses} ${isActive ? 'bg-gray-light' : ''}`}
                >
                  {subHeading.value}
                </a>
              </span>
              ...
              {(subHeading.subHeadings?.length || 0) > 0 &&
                <SubHeadingList subHeadings={subHeading.subHeadings} dampen={true} />
              }
            </li>
          )
        })}
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={toggleMenu}
        >
          <span className="text-base text-current inline-block pr-1">Close Menu</span>
        ...
        {breadcrumbUrl && breadcrumbText &&
          <Link to={breadcrumbUrl} className="flex items-center gap-1 text-lg font-semibold mb-6 ml-6">
              ...
              >
                <span className="block">
                  <a
                    ...
                    `}
                    style={{
                      paddingTop: isPrimaryHeading && index === 0 ? '0px' : '',
                    }}
                  >
                    {heading.value}
                  </a>
                ...
                {(heading.subHeadings?.length || 0) > 0 &&
                  <SubHeadingList subHeadings={heading.subHeadings} isPrimaryHeading={isPrimaryHeading} />
                }
              </li>
            );
          })}
